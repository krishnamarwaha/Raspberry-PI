#! /usr/local/bin/python
	"""
	In this tutorial, we build a robot that responds to human emtions.  We developed a Raspberry Pi Robot with the  that will drive up to you, read your emotions, and then try to have a conversation with you, based on how you’re feeling.  We will show you how you too can build your own DIY emotion-reading robot with a Raspberry Pi.
	...
	distance_from_body = 50	# Distance the  will stop from the human body.
	_speed = 150		# Power of the motors.  Increase or decrease depending on how fast you want to go.
	...
	import atexit
	atexit.register(.stop())	# If we exit, stop the motors
	...
	button_pin = .digitalPort				# Grove Button goes on D11
	distance_sensor_pin = .analogPort		# Ultrasonic Sensor goes on A1
	...
	def main():
		...
		("Hello!  I  am  empathy  bot!")  
	

		while (internet_on() != True):
			("I am waiting on an internet connection!")
			time.sleep(15)
	

		("I am ready to empathize!")  
		while True:
			# Wait for button press.
			wait_for_button()
			take_emotion()
